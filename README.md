...
- **Follows the character you're playing automatically**: a save can hold up to 10 characters, but the tool detects the active one on its own — it's the only slot the game rewrites when it saves — and keeps tracking it, even if you switch characters mid-session. No setup needed; the `-s`/`--slot` option only exists to override this in the rare case it ever guesses wrong.
